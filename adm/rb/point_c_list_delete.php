...
for ($i = 0; $i < $count; $i++) {
    ...
    // 포인트 내역정보
    $sql = " select * from rb_point_c where po_id = '{$po_id}' ";
    $row = sql_fetch($sql);
    ...
    // 포인트 내역삭제
    $sql = " delete from rb_point_c where po_id = '{$po_id}' ";
    sql_query($sql);
    ...
    // 포인트 UPDATE
    $sum_point = get_point_sum_c($_POST['mb_id'][$k]);
    $sql = " update {$g5['member_table']} set rb_point = '$sum_point' where mb_id = '{$str_mb_id}' ";
    sql_query($sql);
}
