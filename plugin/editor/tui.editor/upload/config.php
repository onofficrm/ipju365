...
define("TUI_UPLOAD_IMG_CHECK", 1);  // 이미지 파일을 썸네일 할수 있는지 여부를 체크합니다. ( 해당 파일이 이미지 파일인지 체크합니다. 1이면 사용, 0이면 사용 안함 )
...
function tui_get_file_passname(){
    $tmp_name = tui_get_user_id().$_SERVER['REMOTE_ADDR'];
    $tmp_name = md5(sha1($tmp_name));
    return $tmp_name;
}

function tui_generateRandomString($length = 4) {
...
function tui_replace_filename($filename){

    $ext = pathinfo($filename, PATHINFO_EXTENSION);

    $random_str = tui_generateRandomString(4);

    $passname = tui_get_file_passname();
    
    $file_arr = explode('_', $filename);

    return $passname.'_'.$random_str.'.'.$ext;
}